...
DiffusedOptimal::DiffusedOptimal()
{
}
...
void DiffusedOptimal::setInputCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
{
	_inCloud = cloud;
}

void DiffusedOptimal::setOctree(OctreeXYZ<PointT, LeafContainerT, BranchContainerT > * octree)
{
	_octree = octree;
}

void DiffusedOptimal::setParams(float rmin, float rmax, float rmaxpt, float epsi, float scale)
{
	_rmin = rmin;
	_rmax = rmax;
	_rmaxpt = rmaxpt;
...
bool DiffusedOptimal::setAccumzero()
{
...
tensorType DiffusedOptimal::compute3DBallVote(ColumnVector V, float *weight)
{
...
bool DiffusedOptimal::addBallVote(size_t idx, tensorType ballTensorVote, float lambdaN)
{
...
bool DiffusedOptimal::makeVector(pcl::PointXYZ p_i, pcl::PointXYZ p_j, ColumnVector *V)
{
	ColumnVector temp;
...
void DiffusedOptimal::getnormalizeTensor(size_t idx, float weight)
{
	if (weight == 0.0)
		return;
...
bool DiffusedOptimal::voteAccum(float radius, vector<probfeatnode>&probfeatVal)
{
	...
	for (size_t i = 0; i < _inCloud->points.size(); i++)
	{
		...
		if (_octree->nearestKSearch(searchPoint, probfeatVal[i].optimalScale, pointIdxRadiusSearch, pointRadiusSquaredDistance) > 0)
		{
			weight = 0.0;
...
void DiffusedOptimal::prob_measure(vector <probfeatnode> *probval, vector<tensorType> *aggregated_tensor, vector<int> optimalScale)
{
	...
	for (size_t i = 0; i < _inCloud->points.size(); i++)
	{
		...
		probfeatVal[i].optimalScale = optimalScale[i];
	}

	setAccumzero();
	voteAccum(0, probfeatVal);
...
bool DiffusedOptimal::probMeasure(vector<probfeatnode>&probfeatVal, float radius)
{
...
/*****************************************************************************************
*****************************************************************************************/
void DiffusedOptimal::writeGlyphVars(float radius)
{
...
void DiffusedOptimal::computeSaliencyVals(glyphVars &glyphtemp)
{
	float len = glyphtemp.evals[2] + glyphtemp.evals[1] + glyphtemp.evals[0];
...
void DiffusedOptimal::glyphAnalysis(glyphVars &glyphtemp)
{
...
void DiffusedOptimal::getdiffusionvelocity(Eigen::Vector3f evals, metaVelData *diffVel)
{
...
bool DiffusedOptimal::eigendecomposition(glyphVars &glyphtemp, size_t idx)
{
...
void DiffusedOptimal::filterPointTypes(float radius, std::vector<probfeatnode>&probfeatVal, vector <tensorType> *aggregated_tensor)
{
