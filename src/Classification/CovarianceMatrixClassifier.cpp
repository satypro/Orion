...
void CovarianceMatrixClassifier::setInputCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
{
	_inCloud = cloud;
}

void CovarianceMatrixClassifier::setOctree(OctreeXYZ<PointT, LeafContainerT, BranchContainerT > * octree)
{
	_octree = octree;
}

void CovarianceMatrixClassifier::setParams(float rmin, float rmax, float rmaxpt, float epsi, float scale)
{
	_rmin = rmin;
	_rmax = rmax;
	_rmaxpt = rmaxpt;
...
bool CovarianceMatrixClassifier::setAccumzero()
{
...
bool CovarianceMatrixClassifier::voteAccum(float radius, vector<probfeatnode> &probfeatVal)
{
...
bool CovarianceMatrixClassifier::voteAccum(float radius, probfeatnode probfeatVal, int index)
{
	std::vector<int> pointIdxRadiusSearch;
	std::vector<float> pointRadiusSquaredDistance;
	pcl::PointXYZ searchPoint;
...
void CovarianceMatrixClassifier::prob_measure(probfeatnode *probval, tensorType *tensor, int index)
{
...
void CovarianceMatrixClassifier::prob_measure(vector <probfeatnode> *probval, vector<tensorType> *aggregated_tensor)
{
	double radius, dDeltaRadius;
...
bool CovarianceMatrixClassifier::probMeasure(vector<probfeatnode>&probfeatVal)
{
...
bool CovarianceMatrixClassifier::probMeasure(probfeatnode &probfeatVal, int index)
{
	_glyphData.clear();
	_glyphData.resize(_inCloud->points.size());
...
void CovarianceMatrixClassifier::writeGlyphVars(float radius)
{
...
void CovarianceMatrixClassifier::computeSaliencyVals(glyphVars &glyphtemp, int idx)
{
...
void CovarianceMatrixClassifier::glyphAnalysis(glyphVars &glyphtemp)
{
	double eps = 1e-4;
...
bool CovarianceMatrixClassifier::eigendecomposition(glyphVars &glyphtemp, size_t idx)
{
