...
float current_radius_R = 0.0;
static int fNdx = 0;

void DiffusedNormalVotingClassifier::setInputCloud(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud)
{
	_inCloud = cloud;
}

void DiffusedNormalVotingClassifier::setOctree(OctreeXYZ<PointT, LeafContainerT, BranchContainerT > * octree)
{
	_octree = octree;
}

void DiffusedNormalVotingClassifier::setParams(float rmin, float rmax, float rmaxpt, float epsi, float scale)
{
	_rmin = rmin;
	_rmax = rmax;
	_rmaxpt = rmaxpt;
...
bool DiffusedNormalVotingClassifier::setAccumzero()
{
...
tensorType DiffusedNormalVotingClassifier::compute3DBallVote(ColumnVector V, float *weight)
{
...
bool DiffusedNormalVotingClassifier::addBallVote(size_t idx, tensorType ballTensorVote, float lambdaN)
{
...
bool DiffusedNormalVotingClassifier::makeVector(pcl::PointXYZ p_i, pcl::PointXYZ p_j, ColumnVector *V)
{
	ColumnVector temp;
...
void DiffusedNormalVotingClassifier::getnormalizeTensor(size_t idx, float weight)
{
	if (weight == 0.0)
		return;
...
bool DiffusedNormalVotingClassifier::voteAccum(float radius, vector<probfeatnode>&probfeatVal)
{
...
bool DiffusedNormalVotingClassifier::voteAccum(float radius, probfeatnode probVal, int index)
{
	std::vector<int> pointIdxRadiusSearch;
	std::vector<float> pointRadiusSquaredDistance;
	pcl::PointXYZ searchPoint, tempPoint;
...
void DiffusedNormalVotingClassifier::processPoint(ProcessRequest request, probfeatnode *_probval, tensorType *_accum, int index)
{
	_rmin = request.rmin;
	_rmax = request.rmax;
...
void DiffusedNormalVotingClassifier::process(ProcessRequest request,
...
void DiffusedNormalVotingClassifier::prob_measure(vector <probfeatnode> *probval, vector<tensorType> *aggregated_tensor)
{
...
void DiffusedNormalVotingClassifier::prob_measure(probfeatnode *probval, tensorType *tensor, int index)
{
	double radius, dDeltaRadius;
...
bool DiffusedNormalVotingClassifier::probMeasure(probfeatnode &probfeatVal, float radius, int index)
{
...
bool DiffusedNormalVotingClassifier::probMeasure(vector<probfeatnode>&probfeatVal, float radius)
{
...
void DiffusedNormalVotingClassifier::writeGlyphVars(float radius)
{
...
void DiffusedNormalVotingClassifier::computeSaliencyVals(glyphVars &glyphtemp)
{
	float len = glyphtemp.evals[2] + glyphtemp.evals[1] + glyphtemp.evals[0];
...
void DiffusedNormalVotingClassifier::glyphAnalysis(glyphVars &glyphtemp)
{
...
bool DiffusedNormalVotingClassifier::eigendecomposition(glyphVars &glyphtemp, size_t idx)
{
...
void DiffusedNormalVotingClassifier::filterPointTypes(float radius, std::vector<probfeatnode>&probfeatVal, vector <tensorType> *aggregated_tensor)
{
